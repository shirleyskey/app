...
<?php
            ...
            <div class="col-lg-6 col-7">
              <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">Luận Văn</li>
                </ol>
              </nav>
            </div>
          </div>
    <div style="color:#FFF" class="col-xs-12 col-sm-12 col-md-12 col-lg-12 adminhihi">
		...
		<?php
			$sql = "SELECT * FROM `luan_van`";
			$qr = mysqli_query($conn, $sql);
		 ?>
			<div class="caption">
			<h1 style="text-align: center">QUẢN LÝ LUẬN VĂN</h1>
			</div>
			<hr>
			<div class="row">
...
<br>
<center>
	<button type="button" class="btn btn-success"  data-toggle="modal" data-target="#themadmin">THÊM LUẬN VĂN</button>
</center>
			</div>
              ...
              <div class="modal-dialog modal- modal-dialog-centered modal-sm" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h4 class="text-center" style="color: white">THÊM LUẬN VĂN MỚI</h4>
                  ...
                  <div class="modal-body">
                    ...
                    <center>
                      <button type="button" id="themadminmoi" class="btn btn-success button-update">THÊM LUẬN VĂN MỚI</button>
                    </center>
                  </div>
                </div>
              </div>
...
<div class="modal fade" id="ModalSuaAdmin" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
              <div class="modal-dialog modal- modal-dialog-centered modal-sm" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h4 class="text-center" style="color: white">SỬA THÔNG TIN LUẬN VĂN</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                    <i class="ni ni-fat-remove" style="color: white"></i>
                    </button>
                  </div>
...
<script>
	...
	$(document).ready(function() {
		$('button#xoa').click(function(event) {
			...
			var xoa = confirm("Bạn có thực sự muốn xóa Luận Văn này không?");
			if (xoa == true) {
				if(id == 0){
					alert("Luận Văn Này không Thể Bị Xóa!");
				}
				else 
				{
				    $.ajax({
				    	url: 'xu-ly/luan-van/xoa-luan-van.php',
				    	type: 'POST',
				    	dataType: 'HTML',
				    	data: {id: id},
				    });
		...
		$('button#themadminmoi').click(function(event) {
			$.ajax({
				url: 'xu-ly/luan-van/them-luan-van.php',
				type: 'POST',
				dataType: 'HTML',
				data: {
					ten: $('#ten').val(),
		...
		$('#suaadmin').click(function(event) {
			$.ajax({
				url: 'xu-ly/luan-van/sua-luan-van.php',
				type: 'POST',
				dataType: 'HTML',
				data: {
					idsua: $('#idsua').val(),
