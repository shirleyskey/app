...
<?php
            ...
            <div class="col-lg-6 col-7">
              <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">Danh Sách Coi Thi</li>
                </ol>
              </nav>
            </div>
          </div>
    <div style="color:#FFF" class="col-xs-12 col-sm-12 col-md-12 col-lg-12 adminhihi">
		...
		<?php
			$sql = "SELECT * FROM `coi_thi`";
			$qr = mysqli_query($conn, $sql);
		 ?>
			<div class="caption">
			<h1 style="text-align: center">QUẢN LÝ COI THI</h1>
			</div>
			<hr>
			<div class="row">
				<table class="table table-bordered table-responsive">
					...
					<?php while($row = mysqli_fetch_assoc($qr)){ ?>
						<tr>
							...
							<td><?php echo $row["thi_het_hoc_phan"]?></td>
							<td><?php echo ($row["thi_tot_nghiep"] + $row["thi_het_hoc_phan"] ) ?></td>
							<td align="center">
								<button type="button" id="sua" class="btn btn-warning btn-xs button-sua" title="Sửa" idlop="<?php echo $row["id_lop"]?>" idgv="<?php echo $row["id_giaovien"]?>" idsua="<?php echo $row["id_coi_thi"]?>" ><span class="glyphicon glyphicon-edit"></span>
								<span class="btn-inner--icon"><i class="ni ni-settings"></i></span>
							</button>
  								<button type="button" id="xoa" xoa="<?php echo $row["id_coi_thi"]?>" class="btn btn-danger btn-xs button-sua" title="Xóa"><span class="glyphicon glyphicon-trash"></span>
...
<br>
<center>
	<button type="button" class="btn btn-success"  data-toggle="modal" data-target="#themadmin">THÊM COI THI</button>
</center>
			</div>
              ...
              <div class="modal-dialog modal- modal-dialog-centered modal-sm" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h4 class="text-center" style="color: white">THÊM COI THI MỚI</h4>
                  ...
                  <div class="modal-body">
                    ...
                    <div class="form-group">
                      <div class="input-group">
						...
						<select id="idlop" class="form-control">
                    ...
                    <center>
                      <button type="button" id="themadminmoi" class="btn btn-success button-update">THÊM COI THI MỚI</button>
                    </center>
                  </div>
                </div>
              </div>
...
<div class="modal fade" id="ModalSuaAdmin" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
              <div class="modal-dialog modal- modal-dialog-centered modal-sm" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h4 class="text-center" style="color: white">SỬA THÔNG TIN COI THI</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                    <i class="ni ni-fat-remove" style="color: white"></i>
                    </button>
                  </div>
                  ...
                  <div class="modal-body">
				   ...
				   <div class="form-group">
					 <div class="input-group">
					   ...
					   <input type="text" id="idsua" class="hidden">
					   <select id="idlops" class="form-control">
...
<script>
	...
	$(document).ready(function() {
		$('button#xoa').click(function(event) {
			...
			var xoa = confirm("Bạn có thực sự muốn xóa Coi Thi có id: "+id+" ?");
			if (xoa == true) {
				if(id == 0){
					alert("Coi Thi Này không Thể Bị Xóa!");
				}
				else 
				{
				    $.ajax({
				    	url: 'xu-ly/coithi/xoa-coithi.php',
				    	type: 'POST',
				    	dataType: 'HTML',
				    	data: {id: id},
				    });
		...
		$('button#themadminmoi').click(function(event) {
			$.ajax({
				url: 'xu-ly/coithi/them-coithi.php',
				type: 'POST',
				dataType: 'HTML',
				data: {
					lop: $('#idlop').val(),
		...
		$('#suaadmin').click(function(event) {
			$.ajax({
				url: 'xu-ly/coithi/sua-coithi.php',
				type: 'POST',
				dataType: 'HTML',
				data: {
					idsua: $('#idsua').val(),
